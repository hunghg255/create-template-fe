...
import { intro, outro, confirm, select, spinner, cancel, text, note, isCancel, } from 'unprompts';
...
export const startCli = async () => {
    ...
    const frameworkSelected = templates.find((it) => it.framwork === templateLabel) || {};
    const githubRepo = (await select({
        message: 'Select a variant',
        options: frameworkSelected?.variants.map((item) => ({
    ...
    if (!githubRepo.includes('html-css-js')) {
        ...
        if (isInstallDeps) {
            packageManager = (await select({
                ...
                options: packageManagers.map((item) => ({
                    label: item.color(item.name),
                    value: item.name,
                })),
            }));
    ...
    if (!githubRepo.includes('html-css-js')) {
        ...
        fs.writeFileSync(path.resolve(directory, 'package.json'), JSON.stringify(packageJSON, null, 2), { encoding: 'utf-8' });
        if (isInstallDeps) {
            try {
    ...
    note(`${yellow(`cd ${name}`)}\n${isInstallDeps ? '' : yellow('npm install')}\n${yellow('npm run dev')}`, green('🎉 Done. Now run:'));
    outro(`Problems? ${underline(cyan('https://github.com/hunghg255/create-template-fe/issues'))}`);
    return process.exit(0);
};
